...
quant_xgboost/src/feature_engineering.py 로직을 그대로 이식.
(이 레포는 문제 설정을 이진분류 -> 랭킹으로 바꾸는 게 목적이라, feature 자체는
 기존 실험과 동일하게 유지해야 "모델/objective 변경 효과"만 순수하게 비교할 수 있음)
"""
...
def add_label(df: pd.DataFrame, horizon: int = 5, cost_threshold: float = 0.005) -> pd.DataFrame:
    """
    future_return: 랭킹 모델의 relevance label(순위)을 만드는 근거.
    절대 학습 feature로 쓰면 안 됨 (룩어헤드) -- 랭킹/이진분류 둘 다 동일하게 주의.
    """
