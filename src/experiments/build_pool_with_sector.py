"""
pooled_features_kospi200.csv(BASE)에 섹터 강도 피처를 붙여서 저장.

사용법 (레포 루트에서):
    python -m src.experiments.build_pool_with_sector
"""
...
if __name__ == "__main__":
    ...
    print("섹터 매핑 조회 중...")
    ticker_to_sector = build_sector_map(tickers, market="KOSPI")
    ...
    print(f"섹터 피처 NaN 제거: {before_n}행 -> {len(result)}행 "
          f"(섹터 매핑 실패 종목/초기 rolling window 부족 구간 제외)")

    out_path = DATA_DIR / "pooled_features_kospi200_with_sector.csv"
