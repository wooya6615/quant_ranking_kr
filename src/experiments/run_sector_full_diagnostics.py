"""
COMBINED(BASE+SECTOR) 랭킹 결과를 국면(연도)/종목 집중도까지 한 번에 검증.
(BASE 실험 때는 이 체크를 나중에 따로 했는데, 이번엔 처음부터 세트로 확인)

사용법 (레포 루트에서):
    python -m src.experiments.run_sector_full_diagnostics
"""
...
from src.experiments.run_sector_ablation import load_sector_dataset, FEATURE_COLS_COMBINED
...
if __name__ == "__main__":
    ...
    if year_flag:
        print("→ ⚠️ 특정 연도 하나가 전체 edge의 절반 이상을 차지함 (BASE 때와 동일한 패턴 우려)")
    else:
        print("→ 특정 연도 하나에 쏠리지 않음 (BASE 때보다 개선)")

    # -- 2) 종목별 집중도
    print("\n" + "=" * 80)
    print("=== [2] 종목별 edge 분해 (COMBINED) ===")
    print("=" * 80)
    ...
    if not year_flag and not ticker_flag:
        print("→ 연도/종목 둘 다 분산됨 -- COMBINED 개선은 진짜 일반화 가능한 신호일 가능성이 높음.")
    elif year_flag and ticker_flag:
        print("→ 연도/종목 둘 다 여전히 쏠려있음 -- BASE 때와 동일한 패턴이 COMBINED에서도 반복됨.")
        print("  섹터 피처를 추가해도 근본 문제(국면/테마 의존)는 해결되지 않았다는 뜻.")
    else:
        print("→ 연도/종목 중 한쪽만 개선됨 -- 부분적 개선. 어느 쪽이 남았는지 위 결과로 판단할 것.")